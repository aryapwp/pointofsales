...
public partial class PointofSales {
    ...
    /// soOrderAdd
    /// </summary>
    public static SoOrderAdd soOrderAdd
    {
        get => HttpData.Get<SoOrderAdd>("soOrderAdd")!;
        set => HttpData["soOrderAdd"] = value;
    }

    /// <summary>
    /// Page class for so_order
    /// </summary>
    public class SoOrderAdd : SoOrderAddBase
    {
        // Constructor
        public SoOrderAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SoOrderAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SoOrderAddBase : SoOrder
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "soOrderAdd";
        ...
        // Page name
        public string PageName => "SoOrderAdd";
        ...
        // Constructor
        public SoOrderAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("SoOrderList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "SoOrderView")
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            if (insertRow) {
                try {
                    ...
                    rsnew["SO_ORDER_ID"] = SO_ORDER_ID.CurrentValue!;
                } catch (Exception e) {
                    if (Config.Debug)
                        throw;
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // ORDER_NO
                ORDER_NO.SetDbValue(rsnew, ORDER_NO.CurrentValue);

                // ORDER_DATE
                ORDER_DATE.SetDbValue(rsnew, ConvertToDateTime(ORDER_DATE.CurrentValue, ORDER_DATE.FormatPattern), Empty(ORDER_DATE.CurrentValue));

                // COM_CUSTOMER_ID
                COM_CUSTOMER_ID.SetDbValue(rsnew, COM_CUSTOMER_ID.CurrentValue, Empty(COM_CUSTOMER_ID.CurrentValue));
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("ORDER_NO", out value)) // ORDER_NO
                ORDER_NO.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("ORDER_DATE", out value)) // ORDER_DATE
                ORDER_DATE.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("COM_CUSTOMER_ID", out value)) // COM_CUSTOMER_ID
                COM_CUSTOMER_ID.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("ADDRESS", out value)) // ADDRESS
                ADDRESS.SetFormValue(ConvertToString(value));
        }
