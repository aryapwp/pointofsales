...
public partial class PointofSales {
    ...
    public static void SetupMenus()
    {
        ...
        List<MenuItem> menuItems = [
            new(1, "mi_com_customer", "ComCustomerList", -1, true, false, false, "", "", false, true),
            new(2, "mi_so_item", "SoItemList", -1, true, false, false, "", "", false, true),
            new(3, "mi_so_order", "SoOrderList", -1, true, false, false, "", "", false, true),
        ];
    ...
    protected static dynamic ResolveConnection(string dbid = "DB", string name = "")
    {
        ...
        return GetConnectionType(dbid).ToUpper() switch {
            "MYSQL" => Container.ResolveNamed<DatabaseConnection<MySqlConnection, MySqlDbType>>(dbid + name, new NamedParameter("dbid", dbid)),
        ...
        foreach (AdvancedFilter filter in fld.AdvancedFilters) {
            if (filter.ID == fldVal && filter.Enabled) {
                ...
                if (!Empty(method))
                    ...
                    wrk = ConvertToString(typeof(PointofSales).GetMethod(method)?.Invoke(null, [exp, dbid]));
                break;
            }
        }
        return wrk;
    ...
    */
    ...
    public static object? Invoke(string name, object?[]? parameters = null) =>
        typeof(PointofSales).GetMethod(name)?.Invoke(null, parameters);
    ...
    public static object? GetPropertyValue(string name) =>
        typeof(PointofSales).GetProperty(name, BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy | BindingFlags.Instance)?.GetValue(null);
