...
public partial class PointofSales {
    ...
    /// soItemList
    /// </summary>
    public static SoItemList soItemList
    {
        get => HttpData.Get<SoItemList>("soItemList")!;
        set => HttpData["soItemList"] = value;
    }

    /// <summary>
    /// Page class for so_item
    /// </summary>
    public class SoItemList : SoItemListBase
    {
        // Constructor
        public SoItemList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SoItemList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SoItemListBase : SoItem
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "soItemList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fso_itemlist";
        ...
        public SoItemListBase()
        {
            ...
            MultiDeleteUrl = "SoItemDelete";
            MultiUpdateUrl = "SoItemUpdate";
        ...
        // Page name
        public string PageName => "SoItemList";
        ...
        // Constructor
        public SoItemListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(SO_ITEM_ID.AdvancedSearch.ToJson())); // Field SO_ITEM_ID
            filters.Merge(JObject.Parse(SO_ORDER_ID.AdvancedSearch.ToJson())); // Field SO_ORDER_ID
            filters.Merge(JObject.Parse(ITEM_NAME.AdvancedSearch.ToJson())); // Field ITEM_NAME
        ...
        protected bool RestoreFilterList() {
            ...
            // Field SO_ITEM_ID
            if (filter?.TryGetValue("x_SO_ITEM_ID", out sv) ?? false) {
                SO_ITEM_ID.AdvancedSearch.SearchValue = sv;
                SO_ITEM_ID.AdvancedSearch.SearchOperator = filter["z_SO_ITEM_ID"];
                SO_ITEM_ID.AdvancedSearch.SearchCondition = filter["v_SO_ITEM_ID"];
                SO_ITEM_ID.AdvancedSearch.SearchValue2 = filter["y_SO_ITEM_ID"];
                SO_ITEM_ID.AdvancedSearch.SearchOperator2 = filter["w_SO_ITEM_ID"];
                SO_ITEM_ID.AdvancedSearch.Save();
            }
            ...
            // Field ITEM_NAME
            if (filter?.TryGetValue("x_ITEM_NAME", out sv) ?? false) {
                ITEM_NAME.AdvancedSearch.SearchValue = sv;
                ITEM_NAME.AdvancedSearch.SearchOperator = filter["z_ITEM_NAME"];
                ITEM_NAME.AdvancedSearch.SearchCondition = filter["v_ITEM_NAME"];
                ITEM_NAME.AdvancedSearch.SearchValue2 = filter["y_ITEM_NAME"];
                ITEM_NAME.AdvancedSearch.SearchOperator2 = filter["w_ITEM_NAME"];
                ITEM_NAME.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(SO_ITEM_ID); // SO_ITEM_ID
                UpdateSort(SO_ORDER_ID); // SO_ORDER_ID
                UpdateSort(ITEM_NAME); // ITEM_NAME
                UpdateSort(QUANTITY); // QUANTITY
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""so_item"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""so_item"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""so_item"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fso_itemlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fso_itemlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(SO_ITEM_ID.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""so_item"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("SO_ITEM_ID")); // DN
                CreateColumnOption(option.Add("SO_ORDER_ID")); // DN
                CreateColumnOption(option.Add("ITEM_NAME")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fso_itemsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fso_itemsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fso_itemlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(soItemList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(soItemList.RowCount) + "_so_item");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(soItemList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && soItemList.RowType == RowType.Add || IsEdit && soItemList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // SO_ITEM_ID
                SO_ITEM_ID.HrefValue = "";
                SO_ITEM_ID.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fso_itemsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
