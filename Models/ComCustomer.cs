...
public partial class PointofSales {
    ...
    /// </summary>
    [MaybeNull]
    public static ComCustomer comCustomer
    {
        get => HttpData.Resolve<ComCustomer>("com_customer");
        set => HttpData["com_customer"] = value;
    }

    /// <summary>
    /// Table class for com_customer
    /// </summary>
    public class ComCustomer : DbTable
    {
        ...
        public readonly DbField<MySqlDbType> CUSTOMER_NAME;
        ...
        public ComCustomer()
        {
            ...
            Name = "com_customer";
            Type = "TABLE";
            UpdateTable = "com_customer"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            COM_CUSTOMER_ID = new(this, "x_COM_CUSTOMER_ID", 3, MySqlDbType.Int32) {
                ...
                CustomMessage = Language.FieldPhrase("com_customer", "COM_CUSTOMER_ID", "CustomMsg"),
                IsUpload = false
            };
            ...
            // CUSTOMER_NAME
            CUSTOMER_NAME = new(this, "x_CUSTOMER_NAME", 200, MySqlDbType.VarString) {
                Name = "CUSTOMER_NAME",
                ...
                CustomMessage = Language.FieldPhrase("com_customer", "CUSTOMER_NAME", "CustomMsg"),
                IsUpload = false
            };
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            if (data is IDictionary<string, object> d)
                ...
                throw new ArgumentException("Data must be of anonymous or Dictionary<string, object> or Entities.ComCustomer type", "data");
            row = row.Where(kvp => {
            ...
            try {
                ...
                if (row.ContainsKey("COM_CUSTOMER_ID"))
                    row["COM_CUSTOMER_ID"] = lastInsertedId;
                else
                    row.Add("COM_CUSTOMER_ID", lastInsertedId);
                COM_CUSTOMER_ID.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        private string _sqlKeyFilter => "`COM_CUSTOMER_ID` = @COM_CUSTOMER_ID@";
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "ComCustomerView" => Language.Phrase("View"),
                "ComCustomerEdit" => Language.Phrase("Edit"),
                "ComCustomerAdd" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "ComCustomerView",
                Config.ApiAddAction => "ComCustomerAdd",
                Config.ApiEditAction => "ComCustomerEdit",
                Config.ApiDeleteAction => "ComCustomerDelete",
                Config.ApiListAction => "ComCustomerList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "ComCustomerList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("ComCustomerView", parm);
            else
                url = KeyUrl("ComCustomerView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "ComCustomerAdd";
        ...
        {
            string url = "";
            url = KeyUrl("ComCustomerEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("ComCustomerList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("ComCustomerAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("ComCustomerList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("ComCustomerDelete", parm)); // DN
        }
        ...
        {
            string json = "";
            json += "\"COM_CUSTOMER_ID\":" + ConvertToJson(COM_CUSTOMER_ID.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(COM_CUSTOMER_ID.CurrentValue)) {
                url += "/" + COM_CUSTOMER_ID.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("COM_CUSTOMER_ID", out object? v0)) { // COM_CUSTOMER_ID // DN
                    key = UrlDecode(v0); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[0];
                } else {
                    key = Param("COM_CUSTOMER_ID");
                }
                keysList.Add(key);
            }
            // Check keys
            foreach (var keys in keysList) {
                if (KeyFields.Count > 1 && (!IsArray(keys) || keys.Length != KeyFields.Count))
                    continue;
                if (!IsNumeric(keys)) // COM_CUSTOMER_ID
                    continue;
                result.Add(keys);
            }
            return result;
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    COM_CUSTOMER_ID.CurrentValue = keys;
                else
                    COM_CUSTOMER_ID.OldValue = keys;
        ...
        {
            string pageName = "ComCustomerList";
        ...
        public async Task RenderEditRow()
        {
            ...
            CUSTOMER_NAME.EditValue = CUSTOMER_NAME.CurrentValue;
