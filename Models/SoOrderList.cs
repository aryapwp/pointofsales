...
public partial class PointofSales {
    ...
    /// soOrderList
    /// </summary>
    public static SoOrderList soOrderList
    {
        get => HttpData.Get<SoOrderList>("soOrderList")!;
        set => HttpData["soOrderList"] = value;
    }

    /// <summary>
    /// Page class for so_order
    /// </summary>
    public class SoOrderList : SoOrderListBase
    {
        // Constructor
        public SoOrderList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public SoOrderList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class SoOrderListBase : SoOrder
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "soOrderList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fso_orderlist";
        ...
        public SoOrderListBase()
        {
            ...
            MultiDeleteUrl = "SoOrderDelete";
            MultiUpdateUrl = "SoOrderUpdate";
        ...
        // Page name
        public string PageName => "SoOrderList";
        ...
        // Constructor
        public SoOrderListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(SO_ORDER_ID.AdvancedSearch.ToJson())); // Field SO_ORDER_ID
            filters.Merge(JObject.Parse(ORDER_NO.AdvancedSearch.ToJson())); // Field ORDER_NO
            filters.Merge(JObject.Parse(ORDER_DATE.AdvancedSearch.ToJson())); // Field ORDER_DATE
            filters.Merge(JObject.Parse(COM_CUSTOMER_ID.AdvancedSearch.ToJson())); // Field COM_CUSTOMER_ID
        ...
        protected bool RestoreFilterList() {
            ...
            // Field SO_ORDER_ID
            if (filter?.TryGetValue("x_SO_ORDER_ID", out sv) ?? false) {
                SO_ORDER_ID.AdvancedSearch.SearchValue = sv;
                SO_ORDER_ID.AdvancedSearch.SearchOperator = filter["z_SO_ORDER_ID"];
                SO_ORDER_ID.AdvancedSearch.SearchCondition = filter["v_SO_ORDER_ID"];
                SO_ORDER_ID.AdvancedSearch.SearchValue2 = filter["y_SO_ORDER_ID"];
                SO_ORDER_ID.AdvancedSearch.SearchOperator2 = filter["w_SO_ORDER_ID"];
                SO_ORDER_ID.AdvancedSearch.Save();
            }

            // Field ORDER_NO
            if (filter?.TryGetValue("x_ORDER_NO", out sv) ?? false) {
                ORDER_NO.AdvancedSearch.SearchValue = sv;
                ORDER_NO.AdvancedSearch.SearchOperator = filter["z_ORDER_NO"];
                ORDER_NO.AdvancedSearch.SearchCondition = filter["v_ORDER_NO"];
                ORDER_NO.AdvancedSearch.SearchValue2 = filter["y_ORDER_NO"];
                ORDER_NO.AdvancedSearch.SearchOperator2 = filter["w_ORDER_NO"];
                ORDER_NO.AdvancedSearch.Save();
            }

            // Field ORDER_DATE
            if (filter?.TryGetValue("x_ORDER_DATE", out sv) ?? false) {
                ORDER_DATE.AdvancedSearch.SearchValue = sv;
                ORDER_DATE.AdvancedSearch.SearchOperator = filter["z_ORDER_DATE"];
                ORDER_DATE.AdvancedSearch.SearchCondition = filter["v_ORDER_DATE"];
                ORDER_DATE.AdvancedSearch.SearchValue2 = filter["y_ORDER_DATE"];
                ORDER_DATE.AdvancedSearch.SearchOperator2 = filter["w_ORDER_DATE"];
                ORDER_DATE.AdvancedSearch.Save();
            }

            // Field COM_CUSTOMER_ID
            if (filter?.TryGetValue("x_COM_CUSTOMER_ID", out sv) ?? false) {
                COM_CUSTOMER_ID.AdvancedSearch.SearchValue = sv;
                COM_CUSTOMER_ID.AdvancedSearch.SearchOperator = filter["z_COM_CUSTOMER_ID"];
                COM_CUSTOMER_ID.AdvancedSearch.SearchCondition = filter["v_COM_CUSTOMER_ID"];
                COM_CUSTOMER_ID.AdvancedSearch.SearchValue2 = filter["y_COM_CUSTOMER_ID"];
                COM_CUSTOMER_ID.AdvancedSearch.SearchOperator2 = filter["w_COM_CUSTOMER_ID"];
                COM_CUSTOMER_ID.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(SO_ORDER_ID); // SO_ORDER_ID
                UpdateSort(ORDER_NO); // ORDER_NO
                UpdateSort(ORDER_DATE); // ORDER_DATE
                UpdateSort(COM_CUSTOMER_ID); // COM_CUSTOMER_ID
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""so_order"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""so_order"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""so_order"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fso_orderlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fso_orderlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(SO_ORDER_ID.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""so_order"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("SO_ORDER_ID")); // DN
                CreateColumnOption(option.Add("ORDER_NO")); // DN
                CreateColumnOption(option.Add("ORDER_DATE")); // DN
                CreateColumnOption(option.Add("COM_CUSTOMER_ID")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fso_ordersrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fso_ordersrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fso_orderlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(soOrderList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(soOrderList.RowCount) + "_so_order");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(soOrderList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && soOrderList.RowType == RowType.Add || IsEdit && soOrderList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                SO_ORDER_ID.TooltipValue = "";

                // ORDER_NO
                ORDER_NO.HrefValue = "";
                ORDER_NO.TooltipValue = "";

                // ORDER_DATE
                ORDER_DATE.HrefValue = "";
                ORDER_DATE.TooltipValue = "";

                // COM_CUSTOMER_ID
                COM_CUSTOMER_ID.HrefValue = "";
                COM_CUSTOMER_ID.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fso_ordersrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
