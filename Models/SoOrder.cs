...
public partial class PointofSales {
    ...
    /// </summary>
    [MaybeNull]
    public static SoOrder soOrder
    {
        get => HttpData.Resolve<SoOrder>("so_order");
        set => HttpData["so_order"] = value;
    }

    /// <summary>
    /// Table class for so_order
    /// </summary>
    public class SoOrder : DbTable
    {
        ...
        public readonly DbField<MySqlDbType> SO_ORDER_ID;

        public readonly DbField<MySqlDbType> ORDER_NO;

        public readonly DbField<MySqlDbType> ORDER_DATE;

        public readonly DbField<MySqlDbType> COM_CUSTOMER_ID;

        public readonly DbField<MySqlDbType> ADDRESS;
        ...
        public SoOrder()
        {
            ...
            Name = "so_order";
            Type = "TABLE";
            UpdateTable = "so_order"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // SO_ORDER_ID
            SO_ORDER_ID = new(this, "x_SO_ORDER_ID", 20, MySqlDbType.Int64) {
                Name = "SO_ORDER_ID",
                Expression = "`SO_ORDER_ID`",
                BasicSearchExpression = "`SO_ORDER_ID`",
                DateTimeFormat = -1,
                VirtualExpression = "`SO_ORDER_ID`",
                IsVirtual = false,
                ...
                CustomMessage = Language.FieldPhrase("so_order", "SO_ORDER_ID", "CustomMsg"),
                IsUpload = false
            };
            SO_ORDER_ID.Raw = true;
            Fields.Add("SO_ORDER_ID", SO_ORDER_ID);

            // ORDER_NO
            ORDER_NO = new(this, "x_ORDER_NO", 200, MySqlDbType.VarString) {
                Name = "ORDER_NO",
                Expression = "`ORDER_NO`",
                UseBasicSearch = true,
                BasicSearchExpression = "`ORDER_NO`",
                DateTimeFormat = -1,
                VirtualExpression = "`ORDER_NO`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("so_order", "ORDER_NO", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("ORDER_NO", ORDER_NO);

            // ORDER_DATE
            ORDER_DATE = new(this, "x_ORDER_DATE", 135, MySqlDbType.DateTime) {
                Name = "ORDER_DATE",
                Expression = "`ORDER_DATE`",
                BasicSearchExpression = CastDateFieldForLike("`ORDER_DATE`", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "`ORDER_DATE`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"],
                CustomMessage = Language.FieldPhrase("so_order", "ORDER_DATE", "CustomMsg"),
                IsUpload = false
            };
            ORDER_DATE.Raw = true;
            ...
            // COM_CUSTOMER_ID
            COM_CUSTOMER_ID = new(this, "x_COM_CUSTOMER_ID", 3, MySqlDbType.Int32) {
                Name = "COM_CUSTOMER_ID",
                Expression = "`COM_CUSTOMER_ID`",
                BasicSearchExpression = "`COM_CUSTOMER_ID`",
                DateTimeFormat = -1,
                VirtualExpression = "`COM_CUSTOMER_ID`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"],
                CustomMessage = Language.FieldPhrase("so_order", "COM_CUSTOMER_ID", "CustomMsg"),
                IsUpload = false
            };
            COM_CUSTOMER_ID.Raw = true;
            Fields.Add("COM_CUSTOMER_ID", COM_CUSTOMER_ID);

            // ADDRESS
            ADDRESS = new(this, "x_ADDRESS", 200, MySqlDbType.VarString) {
                Name = "ADDRESS",
                Expression = "`ADDRESS`",
                UseBasicSearch = true,
                ...
                Required = true, // Required field
                SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"],
                CustomMessage = Language.FieldPhrase("so_order", "ADDRESS", "CustomMsg"),
                IsUpload = false
            };
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            if (data is IDictionary<string, object> d)
                ...
                throw new ArgumentException("Data must be of anonymous or Dictionary<string, object> or Entities.SoOrder type", "data");
            row = row.Where(kvp => {
            ...
            try {
                ...
                if (row.ContainsKey("SO_ORDER_ID"))
                    row["SO_ORDER_ID"] = lastInsertedId;
                else
                    row.Add("SO_ORDER_ID", lastInsertedId);
                SO_ORDER_ID.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            if (row == null)
                return;
            try {
                SO_ORDER_ID.DbValue = row["SO_ORDER_ID"]; // Set DB value only
                ORDER_NO.DbValue = row["ORDER_NO"]; // Set DB value only
                ORDER_DATE.DbValue = row["ORDER_DATE"]; // Set DB value only
                COM_CUSTOMER_ID.DbValue = row["COM_CUSTOMER_ID"]; // Set DB value only
        ...
        private string _sqlKeyFilter => "`SO_ORDER_ID` = @SO_ORDER_ID@";
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "SoOrderView" => Language.Phrase("View"),
                "SoOrderEdit" => Language.Phrase("Edit"),
                "SoOrderAdd" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "SoOrderView",
                Config.ApiAddAction => "SoOrderAdd",
                Config.ApiEditAction => "SoOrderEdit",
                Config.ApiDeleteAction => "SoOrderDelete",
                Config.ApiListAction => "SoOrderList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "SoOrderList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("SoOrderView", parm);
            else
                url = KeyUrl("SoOrderView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "SoOrderAdd";
        ...
        {
            string url = "";
            url = KeyUrl("SoOrderEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("SoOrderList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("SoOrderAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("SoOrderList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("SoOrderDelete", parm)); // DN
        }
        ...
        {
            string json = "";
            json += "\"SO_ORDER_ID\":" + ConvertToJson(SO_ORDER_ID.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(SO_ORDER_ID.CurrentValue)) {
                url += "/" + SO_ORDER_ID.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("SO_ORDER_ID", out object? v0)) { // SO_ORDER_ID // DN
                    key = UrlDecode(v0); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[0];
                } else {
                    key = Param("SO_ORDER_ID");
                }
                keysList.Add(key);
            }
            // Check keys
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    SO_ORDER_ID.CurrentValue = keys;
                else
                    SO_ORDER_ID.OldValue = keys;
        ...
        public async Task RenderEditRow()
        {
            ...
            ORDER_NO.EditValue = ORDER_NO.CurrentValue;
        ...
        public async Task ExportDocument(dynamic doc, DbDataReader dataReader, int startRec, int stopRec, string exportType = "")
        {
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    doc.BeginExportRow();
                    if (exportType == "view") {
                        doc.ExportCaption(SO_ORDER_ID);
                        doc.ExportCaption(ORDER_NO);
                        doc.ExportCaption(ORDER_DATE);
                        doc.ExportCaption(COM_CUSTOMER_ID);
            ...
            do { // DN
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            await doc.ExportField(SO_ORDER_ID);
                            await doc.ExportField(ORDER_NO);
                            await doc.ExportField(ORDER_DATE);
                            await doc.ExportField(COM_CUSTOMER_ID);
